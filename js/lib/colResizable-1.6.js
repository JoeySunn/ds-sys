/**
	...
	添加：
		th 中的.th-inner 也加入宽度编号和th一样
		fixed-table-body 设置表格宽度时也设置 fixed-table-body宽度设置
...
(function($){ 	
	...
	var init = function( tb, options){	
		...
		if(!t.is("table") || tables[id] && !t.opt.partialRefresh) {
			syncGrips(tables[id]);//重新渲染拖动条JColResizer
			return;
		...
		t.g = []; t.c = []; t.w = t.width(); t.gc = t.prev(); t.box = t.parent();t.f=t.opt.fixed;	//t.c and t.g are arrays of columns and grips respectively				
	...
	var createGrips = function(t){	
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			var inner=c.find(".th-inner");
			if(inner){
				inner.css("width",c.w+'px');
			}
			
			g.data(SIGNATURE, {i:i, t:t.attr(ID), last: i == t.ln-1});	 //grip index and its table name are stored in the HTML 												
		}); 	
		t.gc.find(".JCLRgrip:last").remove();
	...
	//根据滚动条等设置fixed-table-body 宽度设置
	var boxW=function(w,t){//fixed-table-body 宽度设置
		var box_w=w,sw=t.box.parent().width();
		var boxH=t.box.height();
		var tH=t.height();
		if(boxH<tH){
			box_w+=scrollWidth+2;
		}
	...
	*/
	...
	var onResize = function(){
		for(var t in tables){
            if( tables.hasOwnProperty( t ) && tables[t].closest(".tab-pane").hasClass("active") ) {
                t = tables[t];
                var i, mw=0;
